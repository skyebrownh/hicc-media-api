...
def test_post_schedule(test_client, clean_schedules_table):
    invalid_json1 = {}
    invalid_json2 = {"month_start_date": "INVALID"}
    valid_json = {"month_start_date": "2002-01-01"}
    ...
    response_json = response.json()
    assert response.status_code == 200 
    assert response_json.get("month_start_date") == "2002-01-01"
...
def test_update_schedule(test_client, setup_schedule):
    ...
    valid_json = {"month_start_date": "2007-01-01", "notes": "updated"}

    response1 = test_client.patch(f"/schedules/{setup_schedule_id}", json=empty_json)
    assert response1.status_code == 404 
    assert setup_schedule.get("month_start_date") == "2000-01-01"
    ...
    response_json = response.json()
    assert response.status_code == 200
    assert response_json.get("month_start_date") == "2007-01-01"
